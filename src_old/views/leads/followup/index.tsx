  ...
  import { SourceList, getCategoryList, getSubCategoryList, getBranchList,ProductList,ExecutiveList,StatusList } from '@/services/generalservice';
...
const showStatusFilter = false;  // set to false to hide
    ...
    const fetchLeads = async () => {
      ...
      try {
        ...
        const { data } = await getFollowUpLeadsList(payload);
        setData(data);
      } catch {
 ...
 useEffect(() => {
  ...
  const submitFollowUp = async () => {
    ...
    if (status === '2' && (!followupDate || !followupComment.trim())) {
      toast.error('Please fill in the follow-up date and comment.');
      setLoadingFollowUp(false);
      return;
    }

    if (status === '3' && (!walkinDate || !followupBranch || !followupComment.trim())) {
      toast.error('Please fill in walk-in date, branch, and comment.');
      setLoadingFollowUp(false);
      return;
    }
